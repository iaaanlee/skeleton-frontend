...
export const isStageCompleted = (stage: AnalysisStage, currentStatus: AnalysisStatus): boolean => {
  switch (stage) {
    case 'pending':
      return ['blazepose_processing', 'blazepose_completed', 'llm_processing', 'llm_completed'].includes(currentStatus);
    case 'blazepose_processing':
      return ['blazepose_completed', 'llm_processing', 'llm_completed'].includes(currentStatus);
    case 'blazepose_completed':
      return ['llm_processing', 'llm_completed'].includes(currentStatus);
    case 'llm_processing':
